...
import {
  ...
  AddTaskList,
  TaskComponent,
} from "../components/tabnavigatorComponents";
...
export default function Analysis() {
  ...
  return (
    <View>
      ...
      >
        <StatusBar />
        <Header headerName="Analysis" />
        <SearchBox isVisibleADD={false} />
        <AddTaskList name="Today's Task Analysis" displayI={false} />
        ...
        <View style={{ marginBottom: "200%" }}>
          <Text style={[styles.text, { color: "rgba(57, 54, 54, 1)" }]}>
            Total tasks : {totalTasks.length}
          </Text>
          <Text style={[styles.text, { color: "#2ecc71" }]}>
            Completed : {completedTasks.length}
          </Text>
          <Text style={[styles.text, { color: "#BC3939" }]}>
            Uncompleted : {totalTasks.length - completedTasks.length}
          </Text>
        </View>
      </LinearGradient>
    </View>
