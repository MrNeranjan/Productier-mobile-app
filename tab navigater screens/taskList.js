...
export default function TaskList({ route }) {
  ...
  const category = categaryList.find(
    (category) => category.name === route.params.category.name
  );
  ...
  return (
    ...
    >
      ...
      <ScrollView>
        <KeyboardAvoidingView>
          <AddTaskList name="Scheduled Tasks        " displayI={true} />
          {visible ? (
            category.items.map(createTaskComponent)
          ) : (
            <View>
              <Text
                style={{
