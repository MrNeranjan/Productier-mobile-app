...
function CategoryButton({ id, categoryName }) {
  ...
  function handleRename() {
    ...
    if (categoryExists) {
      alert(
        "A category with this name already exists. Please enter a different name."
      );
      return;
    }
    useStore.setState((state) => {
    ...
    <View>
      <TouchableOpacity onLongPress={handleLongPress} onPress={handlePress}>
        <LinearGradient
          colors={[
            "rgba(180,183,33,1)",
            "rgba(180,183,33,0.481)",
            "rgba(210,211,151,0.30)",
          ]}
          start={{ x: 1, y: 1 }}
          end={{ x: 0, y: 0 }}
...
function SearchBox({ isVisibleADD = true }) {
  ...
  function CreateCategory({ props }) {
    return (
      <CategoryButton key={props.id} id={props.id} categoryName={props.name} />
    );
  }
  ...
  function handleCreateButtonPress() {
    ...
    // Check if a category with the entered name already exists
    const categoryExists = CategoryList.some(
      (category) => category.name.toLowerCase() === inputText.toLowerCase()
    );
        ...
        >
          <View style={styles.centeredView}>
            <View style={styles.modalView}>
              ...
              />
              <View style={{ flexDirection: "row" }}>
                <Button title="Create" onPress={handleCreateButtonPress} />
                <View style={{ marginLeft: 10 }}>
                  <Button
                    title="Cancel"
                    color="red"
  ...
  return (
    <View
      style={[
        ...
        isToday ? styles.hilightedDay : null,
      ]}
    >
      <Text style={isToday ? styles.hilightedtext : styles.dayCom}>{day}</Text>
      <Text style={isToday ? styles.hilightedtext : styles.dayCom}>
        {weekday}
      </Text>
    </View>
  );
...
function Header({ headerName }) {
  ...
  return (
    <View
      style={{
        backgroundColor: "rgba(57, 54, 54, 1)",
        paddingBottom: 20,
    ...
    >
      ...
      >
        ...
        <View
          style={{ borderRadius: 25, borderWidth: 2, borderColor: "#8F816F" }}
        >
          <Image
            source={
              image
                ? { uri: image }
                : require("../assets/sources/loginpages/profilePic.png")
            }
      ...
      >
        ...
        <Text style={styles.dateYear}>
          {" "}
          {dayList[3].month}, {dayList[3].year}
        </Text>
      </View>
      <View
        style={{
      ...
      >
        {dayList.map((date, index) => (
          <CreateDateBar key={index} day={date.day} weekday={date.weekday} />
        ))}
      </View>
    </View>
  );
    ...
    <View style={styles.taskListComponent}>
      ...
      >
        ...
        >
          <View>
            <Text
              style={{ fontFamily: "Inter", fontSize: 17, fontWeight: "bold" }}
            >
              {taskName}
            </Text>
            ...
            {isDisplayDate ? (
              <Text style={{ fontFamily: "Inter", fontSize: 12 }}>{DATE}</Text>
            ) : null}
            {isDisplayDes ? (
              <Text style={{ fontFamily: "Inter", fontSize: 12 }}>
